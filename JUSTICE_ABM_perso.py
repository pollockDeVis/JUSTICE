...
from src.model_abm import ABM_JUSTICE
import matplotlib
...
#TODO APN: warning as error to suppress
...
print("--> Initialisation of JUSTICE model")
scenarios = 2
model = ABM_JUSTICE(
...
with alive_bar(len(model.time_horizon.model_time_horizon), force_tty=True) as bar:
    for timestep in range(len(model.time_horizon.model_time_horizon)):
        ...
        model.stepwise_run( timestep=timestep, endogenous_savings_rate=True) #savings_rate = fixed_savings_rate[:, timestep],
...
data_list = [net_output, cpc, emis, disentangled_utility, damages, abatecost, emission_cutting_rate_temporal]
titles = ['Net Economic Output', 'Consumption per Capita', 'Emissions', 'Disaggregated Utility', 'Economic Damages', 'Abatement Cost', 'Emission rate']
    ...
    with alive_bar(len(data_list)) as bar:
        for i, data in enumerate(data_list):
            ...
            for j in range(region_data.shape[1]):
                sns.lineplot(x=model.time_horizon.model_time_horizon, y=region_data[:, j], ax=axs[i])
