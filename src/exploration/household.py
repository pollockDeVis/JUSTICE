...
class Household():
    DISTRIB_RESOLUTION = 0.01;#Resolution of distribution in CELSIUS
    DISTRIB_MIN_VALUE = -2;#Minimum temperature deviation relative to preindustrial levels in CELSIUS
    DISTRIB_MAX_VALUE = +8;#Maximum temperature deviation relative to preindustrial levels in CELSIUS
    DISTRIB_X_AXIS = np.arange(DISTRIB_MIN_VALUE,DISTRIB_MAX_VALUE,DISTRIB_RESOLUTION);#Support for the distribution
    BELIEF_YEAR_OFFSET = np.array([0,10,50,99]);#The years in the future for which each agent has to have a specific belief of temperature elevation
    N_CLIMATE_BELIEFS = len(BELIEF_YEAR_OFFSET);#Number of beliefs
    DEFAULT_INFORMATION_STD = 0.01;
    P_INFORMATION = 0.5;#Probability to assimilate information at a given timestep
    GAMMA = 0.3;#Imperfect memory parameter
    ...
    def __init__(self, region):
        ...
        self.climate_init_mean_beliefs = np.array([1, 1.5, 1.7, 2.]);#Temperature expected for BELIEF_YEAR_OFFSET years
        self.climate_init_var_beliefs =  np.array([0.01, 0.05,0.15,0.3]);#Confidance in the expectation
        
        distrib_beliefs = np.array([Household.gaussian_distrib(g_mean=self.climate_init_mean_beliefs[i], g_std=self.climate_init_var_beliefs[i]) for i
                            in range(self.N_CLIMATE_BELIEFS)]); #Here range(nb_beliefs) depending on the number of beliefs to be modelled
        norm_coeff = np.sum(distrib_beliefs, axis=1);
        self.climate_distrib_beliefs = np.array([distrib_beliefs[i,:] / norm_coeff[i] for i in range(self.N_CLIMATE_BELIEFS)]);
        
        
        #Initialising Climate worry
        self.internal_climate_worry = 0;#Worry coming from interaction with other individuals
        self.external_climate_worry = 0;#Worry coming from extreme weather events and media
        self.climate_worry = 0;#Aggregation of above worries

        #Initialising Abatement Cost worry
        self.internal_abatement_worry = 0;#Worry coming from interaction with other individuals
        self.external_abatement_worry = 0;#Worry coming from economy and media
        self.abatement_worry = 0;#Aggregation of above worries
        ...
        if self.model_region.id%2==0:
            self.utility_parameters = [-1, +10, +10, -10, 1];#Abatement Costs coeff + Damage Costs coeff + Environent Worry level coeff + Abatement Worry level coeff + Perceived Willingness to contribute
        else:
            self.utility_parameters = [-1, -10, +10, -10, 1];
        self.psi_2 = 2.67*10**-3;#Coefficient for climate damage evaluation (see RICE2013R: psi_2)
        
    def expected_climate_damages(self):
        temp_profile = Household.belief_to_projection(self.climate_distrib_beliefs, self.BELIEF_YEAR_OFFSET);
    ...
    def filtered_climate_information(self):
        ...
        """
        return self.model_region.twolevelsgame_model.justice_model.information_model.global_distrib_flsi
        
    def update_climate_distrib_beliefs(self):
        """
        ...
        if p < self.P_INFORMATION:
            ...
            self.climate_distrib_beliefs = np.array([Household.gaussian_distrib(g_mean=mean_beliefs[i], g_std=self.climate_init_var_beliefs[i]) for i in range(self.N_CLIMATE_BELIEFS)]);
            norm_coeff = np.sum(self.climate_distrib_beliefs, axis=1);
            self.climate_distrib_beliefs=np.array([self.climate_distrib_beliefs[i,:] / norm_coeff[i] for i in range(self.N_CLIMATE_BELIEFS)]);
            #Merging of learned belief and belief with initial std
            self.climate_distrib_beliefs = self.GAMMA * distrib_beliefs_save + (1-self.GAMMA)*self.climate_distrib_beliefs
    ...
    def assess_policy(self):
        ...
        U = self.utility_parameters @ np.array([self.expected_abatement_costs(), self.expected_climate_damages(), self.climate_worry, self.abatement_worry, self.perceived_WTC])
        return  U
    ...
    ###                                 UTILS
    ###########################################################################
    def gaussian_distrib(g_mean=0, g_std=1, min_val=DISTRIB_MIN_VALUE, max_val=DISTRIB_MAX_VALUE, step=DISTRIB_RESOLUTION):
        """
        Parameters
        ----------
    ...
    def mean_distribution(distribution, min_val=DISTRIB_MIN_VALUE, max_val=DISTRIB_MAX_VALUE, step=DISTRIB_RESOLUTION):
        """
        Parameters
        ----------
        distribution : TYPE
