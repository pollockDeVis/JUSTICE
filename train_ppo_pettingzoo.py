...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), self.critic(self.network(x))
    ...
    for iteration in tqdm(range(1, args.num_iterations + 1)):
        ...
        for step in range(0, args.num_steps):
            ...
            if next_done_bool.any():
                ...
                print(f"global_step={global_step}, mean_episodic_return={mean_episodic_return}")
