...
from rl.nn.configs import GraphAttentionBlock
...
class GraphAttention(nn.Module):
    ...
    def __init__(self, graph_config: GraphAttentionBlock, input_dim: int):
        ...
        self.out_features = graph_config.out_features
        self.dropout = graph_config.dropout
        self.negative_slope = graph_config.negative_slope
        self.num_heads = graph_config.number_heads
        self.self_loop_type = graph_config.self_loop_type
        self.average = graph_config.average
        self.normalize = graph_config.normalize
        self.bias = graph_config.bias

        self.W = nn.Parameter(
            torch.zeros(size=(input_dim, self.num_heads * self.out_features))
        )
        self.a_i = nn.Parameter(
            torch.zeros(size=(self.num_heads, self.out_features, 1))
        )
        self.a_j = nn.Parameter(
            torch.zeros(size=(self.num_heads, self.out_features, 1))
        )
        if self.bias:
            if self.average:
                self.bias = nn.Parameter(torch.DoubleTensor(self.out_features))
            else:
                self.bias = nn.Parameter(
                    torch.DoubleTensor(self.num_heads * self.out_features)
                )
        else:
    ...
    def forward(self, X: torch.Tensor, adj: torch.Tensor):
        ...
        Arguments:
            X (tensor): input of the graph attention layer [N * in_features, N: number of agents]
            adj_matrix (tensor): the learned communication graph (adjancy matrix) by the sub-scheduler [N * N]
        ...
        h = torch.matmul(X, self.W).view(-1, self.num_heads, self.out_features)
        N = h.size()[0]
        ...
        if self.normalize:
            if self.self_loop_type != 1:
                attention += 1e-15
            attention = attention / attention.sum(dim=1).unsqueeze(dim=1).expand(
                N, N, self.num_heads
            )
            attention = attention * adj
    ...
    def __repr__(self):
        return self.__class__.__name__ + "(in_features={}, out_features={})".format(
